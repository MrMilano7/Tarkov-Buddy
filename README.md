...
**Current version: v0.9.9** -- Reference maps get a zero-install path too: a "Fetch reference map" button now appears on any Maps card whose RE3MR page is known (Customs, Woods, Interchange, Reserve, Streets of Tarkov to start), for anyone who hasn't run the Termux register_maps.py path. Each visitor's own browser fetches the page, reads its og:image tag to find the actual image (RE3MR's URLs don't follow one predictable pattern -- confirmed the hard way in an earlier session), downloads the image, and stores it in that browser's own IndexedDB. Nothing is redistributed by this app or repo, same NC-license respect as before -- only now every visitor can self-serve instead of only Termux users. If CORS blocks the image fetch (unverifiable until a real browser tries it against reemr.se), the failure is clean -- no partial data ever written -- and the UI surfaces a direct link to open the image manually instead. File-based reference.json (Termux path) and browser-fetched entries merge automatically, file wins on a same-map conflict. A "Remove" option clears a browser-fetched map from local storage.
